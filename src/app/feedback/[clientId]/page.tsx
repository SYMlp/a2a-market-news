...
const DIMENSION_DEFS = [
  { key: 'usability', labelKey: 'dimUsability' as const },
  { key: 'creativity', labelKey: 'dimCreativity' as const },
  { key: 'responsiveness', labelKey: 'dimResponsiveness' as const },
  { key: 'fun', labelKey: 'dimFun' as const },
  { key: 'reliability', labelKey: 'dimReliability' as const },
]
...
export default function FeedbackPage() {
  ...
  const [preview, setPreview] = useState<{ content: string; structured: Record<string, unknown> } | null>(null)
  const [editContent, setEditContent] = useState('')
  const [editRating, setEditRating] = useState(0)
  const [result, setResult] = useState<{ achievements: AchievementItem[]; points: number } | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/circles`)
      ...
      .then(async (circlesRes) => {
        ...
        for (const circle of circlesRes.data as CircleInfo[]) {
          const appsRes = await fetch(`/api/circles/${circle.slug}/apps`).then(r => r.json()).catch(() => null)
          if (appsRes?.success) {
            const found = (appsRes.data as CircleAppCandidate[]).find(
              a => a.clientId === clientId || a.metadata?.clientId === clientId,
            )
            if (found) {
  ...
  const handlePAReview = async () => {
    ...
    try {
      ...
      if (data.success && data.phase === 'preview') {
        ...
        setError(data.error || tf('genFailed'))
        setPhase('idle')
      }
    } catch {
      setError(tf('networkError'))
      setPhase('idle')
    }
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FFFBF5]">
        ...
        <div className="relative text-center space-y-6 max-w-md mx-auto px-4">
          ...
          <h2 className="text-3xl font-extrabold text-gray-800 font-heading">{tf('doneTitle')}</h2>
          <p className="text-orange-500">{tf('doneSubtitle')}</p>

          {result.points > 0 && (
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-full">
              <span className="text-amber-500">+20</span>
              <span className="text-amber-600 text-sm font-semibold">{tf('pointsLabel', { points: result.points })}</span>
            </div>
          )}

          {result.achievements.length > 0 && (
            <div className="space-y-2">
              <p className="text-sm text-gray-400">{tf('unlocked')}</p>
              {result.achievements.map((a: AchievementItem, i: number) => (
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl font-extrabold text-gray-800 mb-2 font-heading">
              {appInfo ? tf('evaluateNamed', { name: appInfo.name }) : tf('evaluateApp')}
            </h2>
            {appInfo && (
          ...
          {phase !== 'manual' && (
            <Card className="p-8 mb-6">
              <div className="text-center mb-6">
                <h3 className="text-xl font-bold text-gray-800 font-heading mb-2">{tf('paSmartTitle')}</h3>
                <p className="text-gray-400 text-sm">{tf('paSmartDesc')}</p>
              </div>
              ...
              {phase === 'idle' && (
                <div className="text-center space-y-4">
                  {user ? (
                    ...
                    <div>
                      <p className="text-gray-400 mb-4">{tf('loginForPa')}</p>
                      <Button asChild>
              ...
              {phase === 'generating' && (
                <div className="text-center py-8 space-y-4">
                  ...
                  <p className="text-orange-500 font-semibold">{tf('paThinking')}</p>
                  <p className="text-gray-400 text-sm">{tf('generatingDesc')}</p>
                </div>
              )}
              ...
              {(phase === 'preview' || phase === 'confirming') && preview && (
                <div className="space-y-6">
                  <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl">
                    <p className="text-orange-600 text-xs font-semibold mb-2">{tf('paScoreLabel')}</p>
                    ...
                    {Boolean(preview.structured?.dimensions) && (
                      <div className="grid grid-cols-5 gap-2 text-center text-xs">
                        {DIMENSION_DEFS.map(d => (
                          <div key={d.key}>
                            <div className="text-gray-400">{tf(d.labelKey)}</div>
                            <div className="text-orange-600 font-bold">
                              {(preview.structured.dimensions as Record<string, number> | undefined)?.[d.key] || '-'}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {phase === 'confirming' ? tf('submitting') : tf('confirmSubmit')}
                    </Button>
                    <button
...
function ManualFeedbackForm({ clientId, onBack }: { clientId: string; onBack: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else setError(data.error || tf('submitFailed'))
    } catch { setError(tf('networkError')) }
  ...
  if (submitted) {
    return (
      <div className="text-center space-y-6 py-12">
        <div className="text-6xl pulse-glow">🐰</div>
        <h2 className="text-3xl font-extrabold text-gray-800 font-heading">{tf('thanks')}</h2>
        <div className="flex gap-4 justify-center">
          <Button onClick={() => setSubmitted(false)} size="sm">{tf('submitAnother')}</Button>
          <Link href="/" className="px-6 py-3 border border-[#E8E0D8] text-gray-500 text-sm rounded-xl hover:border-orange-200 transition-colors">{tf('backHome')}</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <label className="block text-orange-600 text-xs tracking-widest mb-3 font-semibold">{tf('overallRating')}</label>
        ...
        <div>
          <label className="block text-orange-600 text-xs tracking-widest mb-2 font-semibold">{tf('oneLineSummary')}</label>
        ...
        <div>
          <label className="block text-orange-600 text-xs tracking-widest mb-2 font-semibold">{tf('detailsLabel')}</label>
          <textarea rows={4} value={form.details} onChange={e => setForm(f => ({ ...f, details: e.target.value }))}
            ...
            placeholder={tf('detailsPlaceholder')} />
        </div>

        <Button type="submit" disabled={loading || form.overallRating === 0}
          className="w-full py-4 text-center">
          {loading ? tf('submitting') : tf('submitFeedback')}
        </Button>
        </Card>
      </form>
    </div>
