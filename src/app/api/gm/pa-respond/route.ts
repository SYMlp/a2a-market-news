...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ontologyBase = session ? serializeForPA(session) : ''
    const behaviorPA = session ? (buildBehaviorCognition(session, 'pa') ?? '') : ''
    const ontologyContext = [ontologyBase, behaviorPA].filter(Boolean).join('\n\n')

    const recentPaMessages = session ? getPaHistory(session, 3) : []
    const paHistoryBlock = recentPaMessages.length > 0
      ? `\n你之前说过：\n${recentPaMessages.map((m, i) => `${i + 1}. "${m}"`).join('\n')}\n不要重复类似的话。用新的方式回应。`
      : ''
    ...
    if (session && paResponse) {
      recordPaMessage(session, paResponse)
    }
